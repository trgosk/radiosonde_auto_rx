...
class SondeDecoder(object):
    ...
    def generate_decoder_command_experimental(self):
        ...
        if self.sonde_type == "RS41":
            ...
            if self.telem_raw:
                decode_cmd = f"./rs41mod --ptu2 --json --jsnsubfrm1 --json-raw-frame --softin -i {self.raw_file_option} 2>/dev/null"
            else:
            ...
            # M20 decoder
            if self.telem_raw:
                decode_cmd = f"./m20mod --json --json-raw-frame --ptu -vvv --softin -i {self.raw_file_option} 2>/dev/null"
            else:
